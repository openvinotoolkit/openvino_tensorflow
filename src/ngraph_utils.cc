...
static std::string color_string(unsigned int color) {
  std::stringstream ss;
  ss << "#" << std::setfill('0') << std::setw(6) << std::hex << (color & 0xFFFFFF);
  return ss.str();
}
...
std::string GraphToDot(tf::Graph* graph, const std::string& title,
  ...
  unsigned int cluster_bg_colors[num_cluster_colors]{0xF3C300, 0x875692, 0xF38400,
                                                     0xA1CAF1, 0xBE0032, 0xC2B280, 0x848482, 0x008856,
                                                     0xE68FAC, 0x0067A5, 0xF99379, 0x604E97, 0xF6A600,
                                                     0xB3446C, 0xDCD300, 0x882D17, 0x8DB600, 0x654522,
  ...
  std::map<std::string,unsigned int> cluster_color_map;
  ...
  for (auto id = 0; id < graph->num_node_ids(); ++id) {
    ...
    if (annotate_device) {
      ...
      if (node->type_string() == "_Recv" || node->type_string() == "_HostRecv") {
        fill_color = "#f29999";
        style = "rounded,filled";
      }
      else if (node->type_string() == "_Send" || node->type_string() == "_HostSend") {
        fill_color = "#99f299";
        style = "rounded,filled";
      }
      else {
        std::string cluster_name;
        if (tf::GetNodeAttr(node->attrs(), "_ngraph_cluster", &cluster_name) == tf::Status::OK()) {
          unsigned int bg_color;
          unsigned int fg_color;
          if (cluster_color_map.find(cluster_name) == cluster_color_map.end()) {
            bg_color = cluster_bg_colors[seen_cluster_count];
            cluster_color_map[cluster_name] = bg_color;
            if (seen_cluster_count < num_cluster_colors - 1) {
              seen_cluster_count++;
            }
          }
          else {
            bg_color = cluster_color_map[cluster_name];
          }
    ...
    dot_string << ">, shape=rect, style=\"" << style << "\", fontcolor=\"" << text_color << "\", "
