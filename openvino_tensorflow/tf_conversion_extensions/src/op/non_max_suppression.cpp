...
OutputVector translate_non_max_suppression_op(
  ...
  auto axis_scores =
      make_shared<Constant>(ov::element::i64, Shape{2}, vector<int64_t>{0, 1});
  ...
  if (op_type == "NonMaxSuppressionV5") {
    ...
    // check from OCM
    ng_nms = make_shared<NonMaxSuppression>(
        boxes_unsqueezed, scores_unsqueezed, max_output_size, iou_threshold,
    ...
    ng_nms = make_shared<NonMaxSuppression>(
        boxes_unsqueezed, scores_unsqueezed, max_output_size, iou_threshold,
        score_threshold, NonMaxSuppression::BoxEncodingType::CORNER, false,
        ov::element::Type_t::i32);
    // set_node_name(node.get_name(), res);
    // return res->outputs();
  ...
  auto begin = make_shared<Constant>(ov::element::i64, ov::Shape{2},
                                     std::vector<int64_t>({0, 2}));
  // set_node_name(node.get_name(), begin);
  auto end = make_shared<Constant>(ov::element::i64, ov::Shape{2},
                                   std::vector<int64_t>({0, -1}));
  // set_node_name(node.get_name(), end);
  auto ng_nms_selected_indices = make_shared<StridedSlice>(
      ng_nms->outputs()[0], begin, end, std::vector<int64_t>{1, 0},
  ...
  if (backend_name == "CPU") {
    ...
    if (op_type == "NonMaxSuppressionV5") {
      ...
      auto ng_nms_selected_scores = make_shared<StridedSlice>(
          ng_nms->outputs()[1], begin, end, std::vector<int64_t>{1, 0},
          std::vector<int64_t>{1, 0}, std::vector<int64_t>{0, 0},
          std::vector<int64_t>{0, 1});
      // set_node_name(node.get_name(), ng_nms_selected_scores);
      // valid_outputs is 1D tensor in case of OV, and 0D tensor in case of TF
      auto ng_squeeze_axis =
          make_shared<Constant>(ov::element::i32, ov::Shape{}, 0);
      // set_node_name(node.get_name(), ng_squeeze_axis);
      auto valid_outputs =
          make_shared<Squeeze>(ng_nms->outputs()[2], ng_squeeze_axis);
      ...
      // set_node_name(node.get_name(), ng_squeeze_axis);
      auto valid_outputs =
          make_shared<Squeeze>(ng_nms->outputs()[1], ng_squeeze_axis);
      // set_node_name(node.get_name(), valid_outputs);
      // SaveNgOp(ng_op_map, op->name(), valid_outputs);
      res.push_back(valid_outputs->outputs()[0]);
    }
