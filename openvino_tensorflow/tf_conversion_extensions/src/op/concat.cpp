...
OutputVector translate_concat_op(const ov::frontend::NodeContext& node) {
  ...
  if (node.get_op_type() == "ConcatV2") {
    ...
    throw runtime_error(
        "ConversionExtension error: Incorrect operation type (" +
        node.get_op_type() + ")");
  }
  ...
  if (auto constant = std::dynamic_pointer_cast<Constant>(
          ng_const_input.get_node_shared_ptr())) {
    tf_concat_axis_vec = constant->cast_vector<int64_t>();
  } else {
    throw runtime_error(
        "ConversionExtension error: Failed to convert axis to Constant while "
        "converting " +
        node.get_op_type());
  }
