...
OutputVector translate_parallel_dynamic_stitch_op(
    const ov::frontend::NodeContext& node) {
  // format for inputs: [indices1, indices2, ..., indicesN, data1, data2, ...,
  // dataN]
  // so we expect at least 2 input and the total number of inputs must be
  // divisible by 2
  // default_op_checks(node, 2, {"ParallelDynamicStitch", "DynamicStitch"});
  auto in_size = node.get_input_size();
  // TENSORFLOW_OP_VALIDATION(node,
  //                         in_size % 2 == 0,
  //                         "The total number of inputs to DynamicStitch or
  ...
  for (int i = 0; i < N; ++i) {
    ...
    //                         "Only static rank for `indices` input is
    //                         supported.");

    bool valid_input = true;

    if (indices.get_partial_shape().is_static()) {
      auto inp_shape = indices.get_shape();
    ...
    if (valid_input) {
      ...
      if (rank_val < 1) {
        ...
        auto start =
            make_shared<Constant>(ov::element::i32, Shape{1}, rank_val);
        auto stop = make_shared<Constant>(ov::element::i32, Shape{1},
                                          numeric_limits<int>::max());
        auto shape_of_single_element =
            make_shared<Slice>(data_shape, start, stop, const_one);
        auto new_shape = make_shared<Concat>(
            OutputVector{const_minus_one, shape_of_single_element}, 0);
  ...
  auto max_idx = make_shared<ReduceMax>(
      indices, Constant::create(element::i32, {1}, {0}), true);
  ...
  auto result =
      make_shared<ScatterUpdate>(sliced_zeros, indices, update, const_zero);
