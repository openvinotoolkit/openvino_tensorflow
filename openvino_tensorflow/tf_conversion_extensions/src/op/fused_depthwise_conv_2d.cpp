...
OutputVector translate_depthwise_conv_2d_native_op(
  ...
  // H W I 1 M -> I M 1 H W
  auto order = make_shared<Constant>(element::i64, Shape{5},
                                     vector<int64_t>{2, 4, 3, 0, 1});
  auto transposed_filter =
      make_shared<opset8::Transpose>(reshaped_filter, order);
  auto ng_conv_node = make_shared<GroupConvolution>(
      ng_input, transposed_filter, ng_strides, ng_padding_below,
  ...
  if (op_type == "DepthwiseConv2dNative") {
    ...
    FRONT_END_GENERAL_CHECK(vec_str_cmp(fused_ops, {"BiasAdd"}) ||
