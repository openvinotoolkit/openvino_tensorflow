...
OutputVector translate_slice_op(const ov::frontend::NodeContext& node) {
  ...
  if (size_vector.size() != input.get_shape().size()) {
    throw runtime_error(
        "ConversionExtension error (" + node.get_op_type() +
        "): Size vector length is not equal to number of input dimensions.");
  }
  ...
  auto start_const = make_shared<Constant>(
      element::i64, Shape{start_vector.size()}, start_vector);
  auto stop_const = make_shared<Constant>(
      element::i64, Shape{stop_vector.size()}, stop_vector);
  ...
  if (input.get_partial_shape().is_static() && input_dims > 0 &&
      input.get_shape()[0] == 0) {
    auto res = make_shared<Constant>(input.get_element_type(), ov::Shape{0},
                                     std::vector<int>({0}));
