...
#include <iostream>
#include "openvino_tensorflow/grappler/costs/measuring_cost_estimator.h"
...
class CostAnalyzer {
 public:
  ...
  Status GenerateReport(std::ostream& os, bool print_analysis, bool verbose);
  OpPerformanceList GetOpwisePerformanceStats() { return op_perf_; };
  Status AnnotateOpCosts(tensorflow::Graph& graph);
 ...
 private:
  ...
  std::map<std::string, int64_t> op_cost_map_;
};
