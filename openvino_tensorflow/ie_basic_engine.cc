...
void IE_Basic_Engine::infer(
  ...
  for (int i = 0; i < inputs.size(); i++) {
    if (inputs[i] != nullptr) {
      ...
      if (m_device != "MYRIAD" && m_device != "HDDL")
        ...
        if (in_idx < 0) {
          throw std::runtime_error("Input parameter with friendly name " + input_names[i] +
                              " not found in ov::Model");
        }
        m_infer_reqs[0].set_input_tensor(in_idx, tensor);
      else {
  ...
  for (int i = 0; i < hoisted_params.size(); i++) {
    if (hoisted_params[i] != nullptr) {
      ov::Tensor tensor(hoisted_params[i]->get_element_type(), hoisted_params[i]->get_shape(), hoisted_params[i]->get_data_ptr());
      const int param_idx = get_input_idx(param_names[i]);
      if (param_idx < 0) {
        throw std::runtime_error("Hoisted parameter with friendly name " + param_names[i] +
                            " not found in ov::Model");
      }
      m_infer_reqs[0].set_input_tensor(param_idx, tensor);
    }
  }
  ...
  for (int i = 0; i < results.size(); i++) {
    ...
    if (outputs[i] != nullptr) {
      ...
      if (out_idx < 0) {
        throw std::runtime_error("Output with friendly name " + output_names[i] +
                            " not found in ov::Model");
      }
      m_infer_reqs[0].set_output_tensor(out_idx, tensor);
    }
  }
