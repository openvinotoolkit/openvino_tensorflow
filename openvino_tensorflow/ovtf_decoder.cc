...
template <typename T>
void extract_tensor_content(const std::string& tensor_content,
                            ov::Tensor* values) {
  const auto tensor_content_size = tensor_content.size();
  FRONT_END_GENERAL_CHECK(tensor_content_size % sizeof(T) == 0,
                          "Size of tensor_content (", tensor_content_size,
                          ") is not a multiple of ", sizeof(T));

  const T* tensor_values = reinterpret_cast<const T*>(tensor_content.data());
  FRONT_END_GENERAL_CHECK(
      values->get_size() == tensor_content_size / sizeof(T),
      "Size of tensor is not equal to tensor_content size.");
  std::copy(tensor_values, tensor_values + tensor_content_size / sizeof(T),
            values->data<T>());
}

template <typename T>
void extract_compressed_tensor_content(
    const ::tensorflow::TensorProto& tensor_proto, int64_t val_size,
    ov::Tensor* values) {
      ...
      switch (values->get_element_type()) {
        ...
        default:
          FRONT_END_THROW("Encountered unknown element type " +
                          values->get_element_type().get_type_name());
      }
      values_data[i] = val_i;
      val_lastsaved = val_i;
    } else {
...
ov::Any OVTFDecoder::get_attribute(const std::string& name) const {
  ...
  switch (attrs[0].value_case()) {
    ...
    case ::tensorflow::AttrValue::ValueCase::kList: {
      ...
      if (list.tensor_size() || list.func_size())
        FRONT_END_GENERAL_CHECK(false,
                                "Conversion from Tensorflow to OpenVINO data "
                                "type failed: List of tensors/functions type "
                                "for '",
                                name, "' attribute is not supported.");
    ...
    case ::tensorflow::AttrValue::ValueCase::kTensor: {
      ...
      FRONT_END_GENERAL_CHECK(
          pshape.is_static(),
          "Dynamic shapes are not supported for Tensor attribute.");
      const auto& tf_type = tensor_proto.dtype();
      FRONT_END_GENERAL_CHECK(TYPE_MAP().count(tf_type),
                              "Encountered unknown element type " +
                                  DataType_Name(tf_type) +
      ...
      if (!tensor_content.empty() && tensor_proto.has_tensor_shape()) {
        ...
        switch (ov_type) {
          case ov::element::boolean:
            ...
            extract_compressed_tensor_content<bool>(tensor_proto, val_size,
                                                    &res);
            break;
          case ov::element::i32:
            val_size = tensor_proto.int_val_size();
            extract_compressed_tensor_content<int32_t>(tensor_proto, val_size,
                                                       &res);
            break;
          case ov::element::i64:
            val_size = tensor_proto.int64_val_size();
            extract_compressed_tensor_content<int64_t>(tensor_proto, val_size,
                                                       &res);
            break;
          case ov::element::f32:
            val_size = tensor_proto.float_val_size();
            extract_compressed_tensor_content<float>(tensor_proto, val_size,
                                                     &res);
            break;
          case ov::element::f64:
            val_size = tensor_proto.double_val_size();
            extract_compressed_tensor_content<double>(tensor_proto, val_size,
                                                      &res);
            break;
          default:
            FRONT_END_THROW("Encountered unknown element type " +
    ...
    case ::tensorflow::AttrValue::ValueCase::kPlaceholder:
      FRONT_END_GENERAL_CHECK(false,
                              "Conversion from Tensorflow to OpenVINO data "
                              "type failed: Placeholder type for '",
                              name, "' attribute is not supported.");
    case ::tensorflow::AttrValue::ValueCase::kFunc:
      FRONT_END_GENERAL_CHECK(false,
                              "Conversion from Tensorflow to OpenVINO data "
                              "type failed: Function type for '",
                              name, "' attribute is not supported.");
    default:
      FRONT_END_GENERAL_CHECK(
          false, "Conversion from Tensorflow to OpenVINO data type failed.");
  }
}
...
void OVTFDecoder::get_input_node(size_t input_port_idx,
  ...
  if (delim_pos != std::string::npos) {
    ...
    FRONT_END_GENERAL_CHECK(
        !port_id.empty() &&
            std::all_of(port_id.begin(), port_id.end(), ::isdigit),
        "Port id is not specified or not a number. Value: ", port_id);
