...
This demo uses Keras image classification model InceptionV3 (https://keras.io/api/applications/inceptionv3) and ImageNet labels (https://storage.googleapis.com/download.tensorflow.org/data/ImageNetLabels.txt)
...
```
military uniform 0.7517109
mortarboard 0.033067156
academic gown 0.018692799
theater curtain 0.010353806
comic book 0.009676185
```
**Note**: use ```--no_show``` flag to disable the application display window. By default the display window is enabled.

In this case, we are using the image of Admiral Grace Hopper. As you can see, the network correctly spots that she's wearing a military uniform, with a high score of 0.8.
