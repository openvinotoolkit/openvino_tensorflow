...
def _pooling(net, stride, operation):
    ...
    if pooling_type == 'avg':
        net = slim.avg_pool2d(
            net, pooling_shape, stride=stride, padding=padding)
                ...
                with tf.variable_scope('comb_iter_{}'.format(iteration)):
                    ...
                    with tf.variable_scope('left'):
                        h1 = self._apply_conv_operation(
                            h1, operation_left, stride, original_input_left)
                    with tf.variable_scope('right'):
                        h2 = self._apply_conv_operation(
                            h2, operation_right, stride, original_input_right)
    ...
    def _apply_conv_operation(self, net, operation, stride,
        ...
        if 'separable' in operation:
            ...
            if stride > 1 or (input_filters != filter_size):
                net = tf.nn.relu(net)
                net = slim.conv2d(
                    net, filter_size, 1, stride=stride, scope='1x1')
...
class NasNetANormalCell(NasNetABaseCell):
    ...
    def __init__(self, num_conv_filters, drop_path_keep_prob, total_num_cells,
        ...
        super(NasNetANormalCell,
              self).__init__(num_conv_filters, operations, used_hiddenstates,
                             hiddenstate_indices, drop_path_keep_prob,
