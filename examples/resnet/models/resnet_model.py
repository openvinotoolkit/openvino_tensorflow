  ...
  """
    ...
    with tf.variable_scope(name):
        ...
        cnn.conv(
            ...
            bias=None)
        res = cnn.conv(
            depth, 1, 1, 1, 1, activation=None, use_batch_norm=False, bias=None)
        output = shortcut + res
        cnn.top_layer = output
        cnn.top_size = depth
...
def residual_block(cnn, depth, stride, pre_activation):
  ...
  """
    ...
    if pre_activation:
        res = cnn.conv(
            depth, 3, 3, 1, 1, activation=None, use_batch_norm=False, bias=None)
        output = shortcut + res
    else:
        res = cnn.conv(
...
class ResnetModel(model_lib.Model):
    ...
    def get_learning_rate(self, global_step, batch_size):
        ...
        warmup_lr = (rescaled_lr * tf.cast(global_step, tf.float32) / tf.cast(
