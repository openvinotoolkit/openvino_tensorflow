...
parser.add_argument('--select_device', type=str, default='NGRAPH',
                    help='Select device: Currently supports XLA_CPU and XLA_NGRAPH')

parser.add_argument('--log_device_placement', type=bool, default=False,
                    help='Enter True of you want to log the device placement')

parser.add_argument('--save_summary_steps', type=int, default=1,
                    help='Enter number of steps after which summmary should be printed')
...
#    resnet_size=FLAGS.resnet_size, num_classes=train_dataset.num_classes,data_format='channels_first')
...
def input_fn(is_training):
  ...
  image = image_preprocessing_fn(image=image,
                                 output_height=224,#network.default_image_size,
                                 output_width=224,#network.default_image_size,
...
def resnet_model_fn(features, labels, mode):
  ...
  with tf.device('/device:'+FLAGS.select_device+':0'):
      ...
      logits = network(
          inputs=features, is_training=(mode == tf.estimator.ModeKeys.TRAIN))#,data_format='channels_first')
...
def main(unused_argv):
  ...
  config_proto=tf.ConfigProto(inter_op_parallelism_threads=1,log_device_placement=FLAGS.log_device_placement,allow_soft_placement=True)
  run_config=tf.estimator.RunConfig().replace(session_config=config_proto,save_summary_steps=FLAGS.save_summary_steps,tf_random_seed=0)
