...
class TfCnnBenchmarksModelTest(tf.test.TestCase):
    ...
    def testSendRecvVariables(self):
        ...
        if self.extended_tests():
            self._testVariables(
                'parameter_server', local_parameter_device='CPU')
    ...
    def _testVariables(self,
        ...
        params = benchmark_cnn.make_params(
            ...
            if all_reduce_spec == 'nccl' else True,
            use_fp16=use_fp16,
            fp16_loss_scale=2.,
            fp16_vars=fp16_vars,
        ...
        with self.test_session(graph=tf.Graph()):
                ...
                for v in all_vars:
                    ...
                    if v.name.startswith('v/'):
                        ...
                        self.assertEquals(v.name,
                                          'v_1/tower_1/gpu_cached_images:0')
                        self.assertEquals(v.device, '/device:GPU:1')
                    elif v.name in ('images:0', 'labels:0',
                                    'init_learning_rate:0', 'global_step:0',
                                    'loss_scale:0',
            ...
            if summary_verbosity > 0:
                ...
                self.assertEqual(summary_verbosity >= 2,
...
class TfCnnBenchmarksTest(tf.test.TestCase):
    ...
    """
        ...
        labels = np.array(
            [0] * half_batch_size + [1] * half_batch_size, dtype=np.int32)
    ...
    def testForwardOnly(self):
        params = test_util.get_params('testForwardOnly')._replace(
            forward_only=True)
    ...
    def testNoLayers(self):
        params = test_util.get_params('testNoLayers')._replace(
            use_tf_layers=False)
        self._train_and_eval_local(params)
    ...
    def testGradientRepacking(self):
        ...
        self._train_and_eval_local(
            params, skip='eval_and_train_from_checkpoint')
    ...
    def testTraceFileChromeTraceFormat(self):
        ...
        params = test_util.get_params(
            'testTraceFileChromeTraceFormat')._replace(
    ...
    def testTfprofFile(self):
        tfprof_file = os.path.join(self.get_temp_dir(),
    ...
    def testShiftRatio(self):
        ...
        params = benchmark_cnn.make_params(
            ...
            task_index=0)
        self.assertEqual(
            benchmark_cnn.BenchmarkCNN(params).image_preprocessor.shift_ratio,
            0.0)
    ...
    def _test_preprocessing_eval(self, image_height, image_width, output_height,
        ...
        params = benchmark_cnn.make_params()
        new_image = preprocessing.eval_image(image, output_height, output_width,
                                             0, 'bilinear',
                                             params.summary_verbosity)
        with self.test_session() as sess:
            new_image_value = sess.run(new_image)
        self.assertAllEqual(
            new_image_value,
            np.full((output_height, output_width, 3), 128, dtype=np.uint8))
    ...
    def _test_preprocessing_traing(self, image_buf, image_color, output_height,
                                   output_width, bbox, batch_position,
                                   resize_method, distortions,
        ...
        self.assertAllClose(
            new_image_value,
            np.full([output_height, output_width, 3],
                    image_color,
                    dtype=np.float32),
            atol=50.,
            rtol=0.)
    ...
    def _test_learning_rate(self, params,
    ...
    def testLearningRate(self):
        ...
        self._test_learning_rate(params, {0: 1., 10000000: 1.})

        params = params._replace(
            init_learning_rate=1., num_learning_rate_warmup_epochs=5)
    ...
    def testNumBatchesAndEpochs(self):
        params = benchmark_cnn.make_params()
        batches, epochs = benchmark_cnn.get_num_batches_and_epochs(
            params, 10, 100)
...
class VariableUpdateTest(tf.test.TestCase):
  ...
  """
    ...
    """
        ...
        self.assertAllClose(
            actual_losses[:len(expected_losses)],
            expected_losses,
            rtol=rtol,
            atol=0.)

    def _test_variable_updates(self,
                               params,
                               var_updates=('parameter_server', 'replicated')):
        for var_update in var_updates:
            self._test_variable_update(
                params._replace(variable_update=var_update))
    ...
    def testVariousAllReduceSpecs(self):
        ...
        self._test_variable_updates(params, var_updates=('replicated',))
        params = params._replace(
            all_reduce_spec='nccl', compact_gradient_transfer=False)
        self._test_variable_updates(params, var_updates=('replicated',))
...
class VariableMgrLocalReplicatedTest(tf.test.TestCase):
    ...
    def _test_grad_aggregation_with_var_mgr(self, variable_mgr, num_towers,
        ...
        _, agg_device_grads = variable_mgr.preprocess_device_grads(tower_grads)
        ...
        with tf.Session(config=config) as sess:
            ...
            if deferred_grads:
                ...
                feed_dict = {
                    g: 0 for grad_vars in tower_grads for g, _ in grad_vars
                }
    ...
    def _test_grad_aggregation(self, params, num_vars):
        ...
        self._test_grad_aggregation_with_var_mgr(
            bench.variable_mgr, bench.num_gpus, num_vars, deferred_grads)

    def test_grad_aggregation(self):
        base_params = benchmark_cnn.make_params(
            num_gpus=10, variable_update='replicated', use_fp16=True)
        params = base_params
        ...
        params = base_params._replace(
            gradient_repacking=3, variable_consistency='relaxed')
        self._test_grad_aggregation(params, 10)
        params = base_params._replace(
            gradient_repacking=3, compact_gradient_transfer=False)
        self._test_grad_aggregation(params, 10)
        params = base_params._replace(
            variable_consistency='relaxed', compact_gradient_transfer=False)
        self._test_grad_aggregation(params, 10)
        params = base_params._replace(
