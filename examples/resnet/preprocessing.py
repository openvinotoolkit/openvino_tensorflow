...
class BaseImagePreprocess(object):
    ...
    def minibatch(self, dataset, subset, use_datasets, cache_data, shift_ratio):
        raise NotImplementedError('Must be implemented by subclass.')
        ...
        with tf.name_scope('batch_processing'):
            ...
            if use_datasets:
                ds_iterator = data_utils.create_iterator(
                    ...
                    self.parse_and_preprocess, dataset, subset, self.train,
                    cache_data)
            ...
            for split_index in xrange(self.num_splits):
                ...
                images[split_index] = tf.reshape(
                    images[split_index],
                    shape=[
                        self.batch_size_per_split, self.height, self.width,
                        self.depth
                    ])
...
class Cifar10ImagePreprocessor(BaseImagePreprocess):
    ...
    Returns:
      distored image.
    """
        image = tf.image.resize_image_with_crop_or_pad(image, self.height + 8,
                                                       self.width + 8)
    ...
    def _eval_image(self, image):
        ...
        if self.summary_verbosity >= 3:
            tf.summary.image('cropped.image', tf.expand_dims(
                distorted_image, 0))
        return distorted_image
    ...
    def minibatch(self,
        ...
        with tf.name_scope('batch_processing'):
            ...
            min_queue_examples = int(
                dataset.num_examples_per_epoch(subset) *
...
class SyntheticImagePreprocessor(BaseImagePreprocess):
    ...
    def minibatch(self,
        ...
        images = tf.truncated_normal(
            input_shape, dtype=self.dtype, stddev=1e-1, name='synthetic_images')
        labels = tf.random_uniform([self.batch_size],
                                   minval=0,
...
class TestImagePreprocessor(BaseImagePreprocess):
  ...
  """
    ...
    def minibatch(self,
        ...
        fake_images = cnn_util.roll_numpy_batches(self.fake_images,
                                                  self.batch_size, shift_ratio)
        fake_labels = cnn_util.roll_numpy_batches(self.fake_labels,
                                                  self.batch_size, shift_ratio)

        with tf.name_scope('batch_processing'):
            image_slice, label_slice = tf.train.slice_input_producer(
                [fake_images, fake_labels], shuffle=False, name='image_slice')
            raw_images, raw_labels = tf.train.batch([image_slice, label_slice],
                                                    batch_size=self.batch_size,
                                                    name='image_batch')
            images = [[] for _ in range(self.num_splits)]
            labels = [[] for _ in range(self.num_splits)]
