  ...
  """
    ...
    def _do_batch_all_reduce(self, all_device_tensors):
        ...
        for i, tensors_across_devices in enumerate(zip(*all_device_tensors)):
            ...
            group_1_main_device = (
                group_0_main_device + group_size) % num_devices
            ...
            with tf.device(avail_devices[group_0_main_device]):
                group_0_reduced_tensor = _all_reduce_using_copy(
                    group_0_tensors, False)
            ...
            with tf.device(avail_devices[group_1_main_device]):
                group_1_reduced_tensor = _all_reduce_using_copy(
                    group_1_tensors, False)
...
class AllReduceSpecAlgorithm(BatchAllReduceAlgorithm):
    ...
    def _do_batch_all_reduce(self, all_device_tensors):
        ...
        return [
            [t for t, _ in grad_vars] for grad_vars in aggregated_device_grads
        ]
    ...
    if params.all_reduce_spec:
        ...
        if params.local_parameter_device == 'gpu':
            devices_to_reduce_on = [
                '/gpu:%d' % i for i in range(params.num_gpus)
            ]
        else:
...
def _undo_compact_all_device_tensors(all_device_tensors,
    ...
    return _apply_to_all_device_tensors(
        all_device_tensors, apply_func, colocate=False)
...
class _TensorPacker(object):
  ...
  """
    ...
    def _concat_tensors(self, device_tensors):
        ...
        return concatenated_grad, self._concat_tensor_state(
            orig_shapes, orig_sizes)
    ...
    def _split_tensors(self, concatenated_tensor):
        ...
        split_size_last = total_tensor_size - split_size * (
            self._num_splits - 1)
    ...
    def _undo_concat_tensors(self, concatenated_tensor, concat_tensor_state):
        ...
        tensors_with_shapes = [
            tf.reshape(grad, shape) for grad, shape in zip(
                tensors_with_sizes, concat_tensor_state.orig_shapes)
        ]
