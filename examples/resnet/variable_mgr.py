...
class VariableMgrLocalFetchFromPS(VariableMgr):
  ...
  """
    ...
    def create_outer_variable_scope(self, device_num):
        return tf.variable_scope('v0', reuse=bool(device_num))
  ...
  """
    ...
    def create_outer_variable_scope(self, device_num):
        ...
        return tf.variable_scope(
            'v0', reuse=bool(device_num), custom_getter=self._custom_getter)
...
class VariableMgrLocalReplicated(VariableMgr):
  ...
  """
    ...
    def preprocess_device_grads(self, device_grads):
        ...
        grads_to_reduce = [
            [g for g, _ in grad_vars] for grad_vars in device_grads
        ]
...
class VariableMgrDistributedFetchFromPS(VariableMgr):
  ...
  """
    ...
    def create_outer_variable_scope(self, device_num):
        ...
        return tf.variable_scope(
            'v0', reuse=bool(device_num), custom_getter=custom_getter)
...
class VariableMgrDistributedFetchFromStagedPS(
    ...
    def __init__(self, benchmark_cnn):
        super(VariableMgrDistributedFetchFromStagedPS,
    ...
    def savable_variables(self):
        ...
        for v in tf.global_variables():
            assert (v.name.startswith(variable_mgr_util.PS_SHADOW_VAR_PREFIX +
                                      '/v0/') or
                    v.name in ('global_step:0', 'loss_scale:0',
