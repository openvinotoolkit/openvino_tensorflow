...
def _convert_params_to_flags_list(params):
  ...
  """
    return [
        '--%s=%s' % (k, str(v))
        for k, v in six.iteritems(params._asdict())
...
def _spawn_benchmark_processes(output_dir_path, num_workers, num_ps,
  ...
  """
    ...
    tf.logging.info(
        'Outputs of processes will be outputted to: %s' % output_dir)
    ...
    if run_distributed:
        ...
        controller_ports = [
            portpicker.pick_unused_port() for _ in range(num_controllers)
        ]
        ...
        args += [
            ...
            '--worker_hosts=' + ','.join(
                'localhost:%d' % p for p in worker_ports)
        ]
        if num_ps > 0:
            ps_hosts_str = ','.join('localhost:%d' % p for p in ps_ports)
            args.append('--ps_hosts=' + ps_hosts_str)
        else:
            controller_host_str = ','.join(
                'localhost:%d' % p for p in controller_ports)
    ...
    try:
        ...
        for i in range(num_controllers):
            process = _create_task_process('controller', i, args, env,
                                           output_dir)
...
class TfCnnBenchmarksDistributedTest(tf.test.TestCase):
        ...
        def run_fn(run_type, inner_params):
            ...
            if run_type == 'Evaluation':
                ...
                if inner_params.variable_update == 'distributed_replicated':
                    inner_params = inner_params._replace(
                ...
                return _spawn_benchmark_processes(output_dir_path, num_workers,
                                                  num_ps, num_controllers,
                                                  inner_params)
    ...
    def testAllReducePsgpu(self):
        ...
        flags_dict = test_util.get_params(test_name)._replace(
            variable_update='distributed_all_reduce', all_reduce_spec='psgpu#4')
...
class DistributedVariableUpdateTest(tf.test.TestCase):
    ...
    def testVarUpdateCpuAsLocalParamDevice(self):
        ...
        self._test_variable_updates('testVarUpdateCpuAsLocalParamDevice',
                                    params)
