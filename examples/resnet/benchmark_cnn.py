...
flags.DEFINE_integer(
    'batch_group_size', 1, 'number of groups of batches processed in the image '
    'producer.')
flags.DEFINE_integer(
...
flags.DEFINE_float('weight_decay', 0.00004, 'Weight decay factor for training.')
flags.DEFINE_float(
...
flags.DEFINE_integer(
    'save_model_secs', 0, 'How often to save trained models. Pass 0 to disable '
    'checkpoints.')
flags.DEFINE_string(
    'train_dir', None,
...
def benchmark_one_step(sess,
    ...
    need_options_and_metadata = (should_profile or (
...
def get_num_batches_and_epochs(params, batch_size, num_examples_per_epoch):
  ...
  """
    ...
    if params.num_epochs:
        num_batches = int(
            float(params.num_epochs) * num_examples_per_epoch / batch_size)
    else:
...
def get_piecewise_learning_rate(piecewise_learning_rate_schedule, global_step,
  ...
  """
    ...
    return tf.train.piecewise_constant(
        global_step, boundaries, values, name='piecewise_learning_rate')
  ...
  """
    ...
    if params.piecewise_learning_rate_schedule:
        if (params.init_learning_rate or params.learning_rate_decay_factor or
            ...
            raise ValueError(
                'No other learning rate-related flags can be specified '
    ...
    if params.num_learning_rate_warmup_epochs > 0 and (
            params.init_learning_rate or
            params.piecewise_learning_rate_schedule):
        warmup_steps = int(
            num_batches_per_epoch * params.num_learning_rate_warmup_epochs)
        ...
        learning_rate = tf.cond(global_step < warmup_steps, lambda: warmup_lr,
...
class BenchmarkCNN(object):
    ...
    """
        ...
        if (self.params.device == 'cpu' and
                self.params.data_format == 'NCHW' and not self.params.mkl):
        ...
        if ((self.params.num_epochs_per_decay or
             self.params.learning_rate_decay_factor) and
                not (self.params.init_learning_rate and
        ...
        if (self.params.use_fp16 and self.params.fp16_vars and
                ...
                self.params.all_reduce_spec and
        ...
        if (self.params.debugger is not None and
                self.params.debugger != 'cli' and
    ...
    """
        ...
        if self.params.variable_update == 'distributed_all_reduce':
            if self.params.job_name == 'worker':
                ...
                raise ValueError(
                    'unrecognized job name: %s' % self.params.job_name)
    ...
    """
        if self.datasets_use_prefetch:
            (image_producer_ops, enqueue_ops,
             fetches) = (self._build_model_with_dataset_prefetching())
        else:
        ...
        while True:
            self._eval_once(saver, summary_writer, target,
                            local_var_init_op_group, image_producer_ops,
                            enqueue_ops, fetches, summary_op)
    ...
    def _eval_once(self, saver, summary_writer, target, local_var_init_op_group,
        ...
        with tf.Session(
            ...
            Ref_count = 49920
            log_fn(
                'Reference Accuracy @ 1 = %.4f Reference Accuracy @ 5 = %.4f [%d examples]'
                % (Ref_1, Ref_5, Ref_count))
            Valid_1 = 1.0 - np.absolute(accuracy_at_1 - Ref_1) / Ref_1
            Valid_5 = 1.0 - np.absolute(accuracy_at_5 - Ref_5) / Ref_5
            log_fn(
                'Validation @ 1 = %.4f Validation @ 5 = %.4f [%d examples] (1.0 is the ideal case )'
    ...
    """
        ...
        summary_op = tf.summary.merge_all()
        summary_writer = None
        if (is_chief and self.params.summary_verbosity and
        ...
        with sv.managed_session(
            ...
            if self.graph_file is not None:
                ...
                tf.train.write_graph(
                    sess.graph.as_graph_def(add_shapes=True), path, filename,
                    as_text)
            ...
            ) if self.params.tfprof_file else None
            ...
            while not done_fn():
                ...
                summary_str = benchmark_one_step(
                    ...
                    step_train_times, self.trace_filename,
                    self.params.partitioned_graph_file_prefix, profiler,
            ...
            if not global_step_watcher:
                ...
                images_per_sec = (self.num_workers * local_step *
            ...
            if is_chief:
                store_benchmarks({
                    'total_images_per_sec': images_per_sec
                }, self.params)
            # Save the model checkpoint.
            if self.params.train_dir is not None and is_chief:
                checkpoint_path = os.path.join(self.params.train_dir,
                                               'model.ckpt')
                ...
                for group_index in xrange(self.batch_group_size):
                    if not self.use_synthetic_gpu_images:
                        ...
                        put_op = image_producer_stages[device_num].put([
                            images_splits[batch_index],
                            labels_splits[batch_index]
                        ])
    ...
    def _build_model_with_dataset_prefetching(self):
        ...
        for device_num in range(len(self.devices)):
            with self.variable_mgr.create_outer_variable_scope(
                    device_num), tf.name_scope(
                        'tower_%i' % device_num) as name_scope:
                function_buffering_resource = function_buffering_resources[
                    device_num]
                results = self.add_forward_pass_and_gradients(
    ...
    def _build_fetches(self, global_step, all_logits, losses, device_grads,
        ...
        for d, device in enumerate(apply_gradient_devices):
            with tf.device('/job:localhost/replica:0/task:0/device:' +
                           self.params.select_device + ':0'):
            ...
            if self.task_index == 0 and self.params.summary_verbosity >= 1:
                ...
                if self.params.summary_verbosity >= 2:
                    ...
                    log_grads = tf.reshape(
                        tf.log(tf.gather(grads, indices_for_non_zero_grads)),
                        [-1])
    ...
    """
        ...
        for task_num in range(self.num_workers):
            ...
            (image_producer_ops,
             image_producer_stages) = (self._build_image_processing(
    ...
    """
        ...
        for task_num in range(self.num_workers):
            ...
            function_buffering_resources = data_utils.build_prefetch_image_processing(
                ...
                self.params, self.devices, self.dataset)
            ...
            for rel_device_num in range(len(self.devices)):
                ...
                with self.variable_mgr.create_outer_variable_scope(
                    ...
                    task_results = self.add_forward_pass_and_gradients(
                        phase_train, rel_device_num, abs_device_num, None, None,
                        None, function_buffering_resource)
                    if phase_train:
                        losses.append(task_results['loss'])
                        device_grads.append(task_results['gradvars'])
                    else:
        ...
        if self.datasets_use_prefetch and function_buffering_resource is not None:
            with tf.device(self.raw_devices[rel_device_num]):
                ...
                images = tf.reshape(
                    images,
                    shape=[
                        self.batch_size // self.num_gpus, image_size,
                        image_size, self.dataset.depth
                    ])
        else:
            if not self.use_synthetic_gpu_images:
                with tf.device(self.cpu_device):
        ...
        with tf.device('/job:localhost/replica:0/task:0/device:' +
            ...
            if rel_device_num == len(self.devices) - 1:
                ...
                if self.params.single_l2_loss_op:
                    ...
                    reshaped_params = [
                        tf.reshape(p, (-1,)) for p in fp32_params
                    ]
            ...
            if self.params.variable_update == 'horovod':
                ...
                grads = [
                    hvd.allreduce(grad, device_dense=horovod_device)
                    for grad in grads
                ]
