...
def get_var_shapes(model):
    ...
    with tf.Graph().as_default():
        ...
        return [
            [int(d) for d in v.shape.dims] for v in tf.trainable_variables()
        ]
...
def all_reduce(all_device_tensors, variable_mgr):
  ...
  """
    ...
    _, aggregated_tower_grads = variable_mgr.preprocess_device_grads(
        tower_grads)
    return [[g
             for g, _ in agg_device_tensors]
    ...
    for device, device_tensors in zip(tower_devices, all_device_tensors):
        with tf.device(device):
            for t in device_tensors:
                ...
                var = tf.Variable(
                    tf.placeholder(tf.float32, t.shape), collections=[])
...
def build_graph(tower_devices, tensor_shapes, variable_mgr, num_iters):
  ...
  """
    ...
    benchmark_op = build_all_reduce_iterations(
        all_device_tensors, tower_devices, variable_mgr, num_iters)
