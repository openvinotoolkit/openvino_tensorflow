...
parser.add_argument('--select_device', type=str, default='NGRAPH',
                    help='Select device: Currently supports XLA_CPU and NGRAPH')
...
def train_preprocess_fn(image, label):
  ...
  image = tf.random_crop(image, [HEIGHT, WIDTH, DEPTH],seed=0)
...
def cifar10_model_fn(features, labels, mode):
  ...
  with tf.device('/job:localhost/replica:0/task:0/device:'+FLAGS.select_device+':0'):
      network = resnet_model.cifar10_resnet_v2_generator(
          FLAGS.resnet_size, NUM_CLASSES ,data_format='channels_first')
...
def main(unused_argv):
  ...
  for cycle in range(FLAGS.train_steps // FLAGS.steps_per_eval):
    ...
    tensors_to_log['loss ']='cross_entropy'
