...
def check_training_outputs_are_reasonable(testcase,
                                          training_outputs,
  ...
  """
    ...
    for lines in train_logs_from_ckpt:
        ...
        if check_output_values:
            check_training_outputs_are_reasonable(
                testcase,
                train_outputs_from_ckpt,
                print_training_accuracy,
                max_final_loss=max_final_loss,
                previous_final_loss=initial_train_outputs[-1].loss)
    # Ensure a new checkpoint was written out.
    testcase.assertNotEqual(train_dir_entries, set(
        os.listdir(params.train_dir)))
...
def manually_compute_losses(numpy_inputs, inputs_placeholder, loss, num_workers,
  ...
  """
    ...
    reported_loss = (loss if params.loss_type_to_report == 'base_loss' else
                     total_loss)
    ...
    grad_vars = opt.compute_gradients(
        total_loss,
        grad_loss=tf.constant(gradient_multiplier, dtype=tf.float32))
    ...
    placeholder_grad_vars = [
        (tf.placeholder(g.dtype, g.shape), v) for g, v in grad_vars
    ]
    placeholder_grads = [g for g, _ in placeholder_grad_vars]
    apply_grads_op = opt.apply_gradients(placeholder_grad_vars)

    batch_iterators = [
        _worker_batches_in_numpy_array(
            numpy_inputs, batch_size, shift_ratio=i / num_workers)
    ...
    with tf.Session(config=tf.ConfigProto(device_count={'GPU': 0})) as sess:
        ...
        for i in range(params.num_batches):
            ...
            for j in range(num_workers):
                ...
                worker_loss, worker_grads = sess.run(
                    (reported_loss, grads), {inputs_placeholder: batch_feed})
  ...
  """
    ...
    def __init__(self):
        super(TestModel, self).__init__(
            'test_model', image_size=1, batch_size=1, learning_rate=1)
    ...
    def manually_compute_losses(self, inputs, num_workers, params):
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            inputs_placeholder = tf.placeholder(
                tf.float32, (None, 1, 1, 1), name='inputs_placeholder')
