...
* Modify the graphdef and dump TB file in ```./vis``` using encapsulate information in ```nodemap.pkl```: ```python ovtf_graph_viewer.py -c nodemap.pkl ./path/to/original_network_pbtxtfile.pbtxt ./vis```. If you do not have the pbtxt of the original tensorflow graph, you can dump it from your script using [write_graph](https://www.tensorflow.org/api_docs/python/tf/train/write_graph)
...
* In your script, import openvino_tensorflow by using: ```import openvino_tensorflow```
* Disable ngraph by calling: ```openvino_tensorflow.disable()```
* Enable ngraph by calling: ```openvino_tensorflow.enable()```
* Check whether ngraph is enabled by calling: ```ngraph.is_enabled()```
* You need to enable ngraph every time you called ```openvino_tensorflow.disable()```, so it is good to check whether ngraph is enabled by calling ```ngraph.is_enabled()```
...
A `ovtf_graph_viewer` script can convert a protobuf (`pb` or `pbtxt`) into a 
...
Run the following for detailed help:
    ...
    python ovtf_graph_viewer.py -h


### Examples for command-line use

* `pbtxt` to TB: ```python ovtf_graph_viewer.py pbtxtfile.pbtxt ./vis```
* `pbtxt` to dot: ```python ovtf_graph_viewer.py -v 0 pbtxtfile.pbtxt ./vis```
* `pb` to TB: ```python ovtf_graph_viewer.py -b pbtxtfile.pb ./vis```
* `pb` to dot: ```python ovtf_graph_viewer.py -b -v 0 pbtxtfile.pb ./vis```
* `pbtxt` to TB (after prepending cluster information): ```python ovtf_graph_viewer.py -c nodemap.pkl pbtxtfile.pbtxt ./vis``` 


### Some other use cases

* graphdef to dot: ```from ovtf_graph_viewer import graphdef_to_dot```
* graphdef to TB: ```from ovtf_graph_viewer import graphdef_to_tensorboard```
* pbtxt to TB: ```from ovtf_graph_viewer import protobuf_to_grouped_tensorboardt; protobuf_to_grouped_tensorboard(input_filename, dot_dir, input_binary=False)```
* pbtxt to dot: ```from ovtf_graph_viewer import protobuf_to_dot; protobuf_to_dot(input_filename, dot_dir, input_binary=False)```
* pb to TB: ```from ovtf_graph_viewer import protobuf_to_grouped_tensorboardt; protobuf_to_grouped_tensorboard(input_filename, dot_dir, input_binary=True)```
* pb to dot: ```from ovtf_graph_viewer import protobuf_to_dot; protobuf_to_dot(input_filename, dot_dir, input_binary=True)```
* pb to graphdef: ```from ovtf_graph_viewer import load_file; load_file(input_filename, input_binary=True)```
* pbtxt to graphdef: ```from ovtf_graph_viewer import load_file; load_file(input_filename, input_binary=False)```
* modify a graphdef's nodes names: ```from ovtf_graph_viewer import modify_node_names; modify_node_names(graph_def, node_map={"net1/node1":"e1/net1/node1"})```
...
[ovtf_graph_viewer]: https://github.com/openvinotoolkit/openvino_tensorflow/tree/master/diagnostics/ovtf_graph_viewer.py
