...
4. [下载并安装 OpenVINO™ Toolkit 2022.2.0 LTS for Windows](https://www.intel.com/content/www/us/en/developer/tools/openvino-toolkit-download.html)
...
- 在最新的**OpenVINO™ integration with TensorFlow**发行版本(version 2.0.0)已经移除了对早于2022.2.0版本的OpenVINO™的支持。如果要使用之前版本的OpenVINO™，请使用**OpenVINO™ integration with TensorFlow**的相应分支。
- 如果要使用OpenVINO™的主分支，请把"openvino_version"参数的值设为"master"。**OpenVINO™ integration with TensorFlow**的构建脚本将会从OpenVINO™主分支上拉取最新的提交。所有openvino-tensorflow的API和功能与基于OpenVINO™ 2022.2.0 的版本一致。
...
因此依赖指定TensorFlow版本的 **OpenVINO™ integration with TensorFlow** wheel将无法兼容TensorFlow out-of-the-box过去的副版本（示例：依赖TF 2.9.2的PyPi openvino-tensorflow 2.2.0无法兼容PyPi TensorFlow 2.6.0）。

请注意PyPi openvino-tensorflow 2.2.0仍兼容TensorFlow PATCH版本。例如，针对 TF 2.9.2构建的openvino-tensorflow将兼容像TF 2.9.0和2.9.2 PATCH版本。 这是因为TensorFlow PATCH版本中完成的安全与漏洞修复不影响其C++运行时库的二进制接口。
