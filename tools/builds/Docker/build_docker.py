...
for bo in Soup.find_all(['item']):
    ...
    base = open("Dockerfile.ubuntu20.04_base", "r")
...
for bo in Soup.find_all(['item']):
    command = ("docker build --pull --rm %s %s %s %s -t %s:latest -f %s  ." %(DOCKER_HTTP_PROXY,DOCKER_HTTPS_PROXY,DOCKER_NO_PROXY,DOCKER_FTP_PROXY,bo.find(['name']).text,bo.find(['name']).text))
    print(command)
    output=subprocess.run(command,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if os.path.exists(bo.find(['name']).text+".log"):
        os.remove(bo.find(['name']).text+".log")
    logging = open(bo.find(['name']).text+".log","w")
    ...
    success_build[bo.find(['name']).text]=success
