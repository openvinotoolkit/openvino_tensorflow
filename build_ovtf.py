...
def main():
    ...
    if (arguments.openvino_version not in ["master", "2022.1.0", "2022.2.0", "2022.3.0"]):
        raise AssertionError(
            "Only 2022.1.0, 2022.2.0, and 2022.3.0 branch of OpenVINO are supported"
        )
    ...
    if arguments.use_tensorflow_from_location != "":
        ...
        if (platform.system() == "Windows"):
            if tags.interpreter == "cp39":
                command_executor([
                    "pip", "install", "--force-reinstall",
                    "https://github.com/openvinotoolkit/openvino_tensorflow/releases/download/v2.3.0/tensorflow-2.10.0-cp39-cp39-win_amd64.whl"
                ])
            else:
