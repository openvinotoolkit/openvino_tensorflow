...
To use the `openvino-tensorflow-addon`, activate the following `virtualenv` to start using nGraph with TensorFlow. 
...
Once you have installed Intel<sup>®</sup> OpenVINO™ Add-on for TensorFlow, you can use TensorFlow to run inference using a trained model.
...
More detailed examples on how to use OpenVINO TensorFlow Add-on are located in the [examples] directory.
