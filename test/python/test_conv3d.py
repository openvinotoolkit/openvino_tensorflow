...
class Conv3DTest(NgraphTest, test.TestCase):
        ...
        for data_format, use_gpu in GetTestConfigs():
            for dtype in self._DtypesToTest(use_gpu):
                ...
                with self.cached_session(use_gpu=use_gpu):
                    ...
                    t1_ph = tf.compat.v1.placeholder(
                        dtype, shape=tensor_in_sizes)
                    t1 = constant_op.constant(
                        x1, shape=tensor_in_sizes, dtype=dtype)
                    t2_ph = tf.compat.v1.placeholder(
                        dtype, shape=filter_in_sizes)
                    t2 = constant_op.constant(
                        x2, shape=filter_in_sizes, dtype=dtype)
                    ...
                    self.assertAllClose(
                        expected, value.flatten(), atol=tol, rtol=tol)
    ...
    def testConv3D1x1x1Filter(self):
        expected_output = [
            ...
            0.62962963, 0.77777778, 0.92592593, 0.85185185, 1.05555556,
            1.25925926, 1.07407407, 1.33333333, 1.59259259, 1.2962963,
            1.61111111, 1.92592593
        ]
    ...
    def _RunAndVerifyBackprop(self, input_sizes, filter_sizes, output_sizes,
        ...
        default_dilations = (dilations[0] == 1 and dilations[1] == 1 and
                             dilations[2] == 1)
        ...
        with self.cached_session(use_gpu=use_gpu) as sess:
            ...
            actual_sess_fn = lambda sess: sess.run(
                actual_grad, feed_dict={
                    t1_ph: t1.eval(),
                    t2_ph: t2.eval()
                })
