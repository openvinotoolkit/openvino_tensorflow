...
class Conv3DTest(NgraphTest, test.TestCase):
    ...
    for data_format, use_gpu in GetTestConfigs():
      for dtype in self._DtypesToTest(use_gpu):
        ...
        with self.cached_session(use_gpu=use_gpu):
          ...
          t1_ph = tf.compat.v1.placeholder(dtype, shape=tensor_in_sizes)
          t1 = constant_op.constant(x1, shape=tensor_in_sizes, dtype=dtype)
          t2_ph = tf.compat.v1.placeholder(dtype, shape=filter_in_sizes)
          ...
          conv = nn_ops.conv3d(t1_ph, t2_ph, strides, padding=padding,
                              data_format=data_format)
          if data_format == "NCDHW":
            conv = test_util.NCHWToNHWC(conv)

          sess_fn = lambda sess: sess.run(conv, feed_dict={t1_ph: t1.eval(), t2_ph: t2.eval()})
  ...
  def _RunAndVerifyBackprop(self, input_sizes, filter_sizes, output_sizes,
    ...
    with self.cached_session(use_gpu=use_gpu) as sess:
      ...
      actual_grad = gradients_impl.gradients(actual, t1_ph
                                              if mode == "input" else t2_ph)[0]
      ...
      actual_sess_fn = lambda sess: sess.run(actual_grad, feed_dict={t1_ph: t1.eval(), t2_ph: t2.eval()})
