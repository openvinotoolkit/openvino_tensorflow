...
class TestMaxPoolBackpropInput(NgraphTest):
  ...
  grad_nchw = {"VALID": np.transpose(grad_nhwc["VALID"], NHWC_TO_NCHW),
  ...
  def test_nhwc(self, padding):
    ...
    with self.device:
      a = max_pool_grad(self.input_nhwc, output, grad, ksize, strides,
                        padding=padding, data_format="NHWC")
      with self.session as sess:
        result = sess.run(a, feed_dict={grad: np_nhwc})

    with tf.device('/cpu:0'):
      b = max_pool_grad(self.input_nhwc, output, grad, ksize, strides,
                        padding=padding, data_format="NHWC")
      with self.session as sess:
  ...
  def test_nchw(self, padding):
    ...
    with self.device:
      a = max_pool_grad(self.input_nchw, output, grad, ksize, strides,
