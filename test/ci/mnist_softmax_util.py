...
def maybe_download_MNIST_data(
        data_download_script_path,
        data_dir
        ):
    """
...
def compute_accuracy(
  ...
  accuracy_value = tf_session.run(
    accuracy, feed_dict={op_x:  mnist_data_set.test.images,
...
def train_model(
  ...
  for i in range(num_training_iterations):
    ...
    if (i == num_training_iterations - 1) and (final_iteration_chrome_trace_filename_or_None is not None):
...
def train_and_evaluate_model(
  ...
  Typical values for tf_device_str_or_None:
    ...
    '/job:localhost/replica:0/task:0/device:XLA_NGRAPH:0'
  ...
  for env_var in [
    ...
    if env_var in os.environ:
        log_stream.write('WARNING: Environment var will influence device placement: {}={}\n'.format(
