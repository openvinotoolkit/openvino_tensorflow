...
# Directory where the TF wheel can be found
...
if [ ! -d "${WHEEL_BUILD_DIR}" ] ; then
    ( >&2 echo 'Cannot find the Wheel build directory.  The wheel build directory must exist when running this script.' )
    exit 1
fi

if [ ! -f "${USER_PLUGIN_PATH:-}" ] ; then
    ( >&2 echo 'USER_PLUGIN_PATH has not been set to point to libngraph_plugin.so' )
    exit 1
fi
...
# USER_PLUGIN_PATH should exist at this point, as it has been checked for above
export TF_ROOT="${ngtf_dir}"
"${bridge_dir}/test/ci/run-premerge-ci-checks.sh"

xtime="$(date)"
echo ' '
echo "===== Completed Tensorflow Testing at ${xtime} ====="
echo ' '
